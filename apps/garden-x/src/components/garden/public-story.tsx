...
import { formatDate, localizedEventLabel } from "@/lib/garden-logic";
    ...
    <article className="min-h-screen bg-background text-foreground">
      <header className="relative min-h-[70svh] overflow-hidden bg-secondary">
        ...
        <div className="absolute inset-x-0 top-0 mx-auto flex max-w-5xl items-center justify-between px-5 py-6 text-primary-foreground sm:px-8">
          ...
          <span className="text-xs opacity-75">{preview ? ui(language, "publicStoryPreview") : ui(language, "sharedPlantStory")}</span>
        </div>
        <div className="absolute inset-x-0 bottom-0 mx-auto max-w-5xl px-5 pb-10 text-primary-foreground sm:px-8 sm:pb-14">
          <p className="eyebrow opacity-75">{ui(language, "recordedHistory")}</p>
      ...
      <div className="mx-auto max-w-3xl px-5 py-14 sm:px-8 sm:py-20">
        <div className="mb-12 max-w-xl">
          <p className="eyebrow">{ui(language, "selectedHistory")}</p>
          <h2 className="mt-2 font-display text-3xl font-light leading-tight sm:text-4xl">{ui(language, "momentsFromLife")}</h2>
          <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{ui(language, "storyBody")}</p>
        </div>
          ...
          {story.moments.map((moment, index) => (
            <section key={moment.id} className="grid gap-4 sm:grid-cols-[7rem_minmax(0,1fr)] sm:gap-8">
              <div className="pt-1">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{ui(language, "dayLabel")} {Math.max(1, story.plant.plantedDaysAgo - moment.daysAgo)}</p>
              </div>
              <div>
                {moment.kind === "photo" ? (
                  <figure>
                  ...
                  <div className="border-l border-border pl-5 sm:pl-7">
                    <p className="eyebrow">{localizedEventLabel(moment.event.type, language)}</p>
      ...
      <footer className="border-t border-border/70 px-5 py-12 text-center sm:px-8">
        ...
        <p className="mt-3 font-display text-lg">{ui(language, "recordedInGarden")}</p>
        <p className="mt-1 text-xs text-muted-foreground">{ui(language, "realPlantStory")}</p>
      </footer>
    </article>
  );
}
