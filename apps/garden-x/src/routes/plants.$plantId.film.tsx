  ...
  return <div className="rise pb-20">
    ...
    <div className="grid gap-8 px-5 sm:px-8 lg:grid-cols-[minmax(0,3fr)_minmax(19rem,1fr)] lg:px-12">
      ...
      <aside>
        ...
        {step === "preview" ? <div><SectionTitle>Ready to remember</SectionTitle><div className="surface overflow-hidden">{current ? <PhotoImage photo={current} alt="" className="aspect-video w-full object-cover" loading="eager" /> : <div className="aspect-video bg-secondary" />}<div className="p-4"><p className="font-display text-xl">{plant.name}</p><dl className="mt-3 grid grid-cols-2 gap-3 text-xs"><div><dt className="text-muted-foreground">Moments</dt><dd>{sequence.length}</dd></div><div><dt className="text-muted-foreground">Duration</dt><dd>About {sequence.length * 3} sec</dd></div><div><dt className="text-muted-foreground">Music</dt><dd>{music}</dd></div><div><dt className="text-muted-foreground">Framing</dt><dd>Portrait crop</dd></div></dl><label className="mt-4 flex items-center justify-between text-sm"><span>Titles</span><input type="checkbox" checked={titles} onChange={(event) => setTitles(event.target.checked)} className="accent-[var(--color-primary)]" /></label></div></div><Button className="mt-5 w-full rounded-full" onClick={saveFilm}><Save /> {saving ? "Saving…" : "Save Growth Film"}</Button></div> : null}
