          ...
          <div className="rise max-w-4xl">
            ...
            <ol className="space-y-5">
              {history.map((e, i) => {
                ...
                return (
                  <li key={e.id} className={cn("grid gap-5 border-t border-border/70 pt-5", e.milestone && photo && "sm:grid-cols-[minmax(0,1fr)_minmax(15rem,0.8fr)] sm:items-center", !e.milestone && "ml-3 border-l border-t-0 py-1 pl-5")}>
                    ...
                    {e.milestone && photo ? <figure className="overflow-hidden rounded-2xl shadow-soft"><button type="button" onClick={() => photoViewer.open(photo.src, photo.caption)} aria-label={`View ${photo.caption} larger`} className="block w-full"><img src={photo.src} alt={photo.caption} loading="lazy" className="aspect-[4/3] w-full object-cover" /></button><figcaption className="bg-card px-3 py-2 text-xs text-muted-foreground">{photo.caption}</figcaption></figure> : null}
                    {i === history.length - 1 ? (
