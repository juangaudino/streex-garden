...
const files = [
  'index.html', 'styles.css', 'language.css', 'visual.css', 'inventory-neighbors.css',
  'garden-labs.css', 'app.js', 'seed-purchase-date.js', 'demo-shell.js', 'manifest.json',
  'service-worker.js', 'supabase-lab-transport.js', 'sites-storage-bootstrap.js', 'sites-storage-v1.js',
  'machines-v1.js', 'machines-v1.css', 'homegrown-source-comparison.js', 'homegrown-source-comparison.css',
  'expansion-batch-a1.js', 'expansion-batch-b1.js'
];
...
const sourceIcon = await readFile(resolve(source, 'assets/lab-icon-approved-512.jpg'));
if (sourceIcon.length < 10000 || sourceIcon[0] !== 0xff || sourceIcon[1] !== 0xd8) throw new Error('Garden Labs approved source icon is missing or is not a valid JPEG payload.');
const purchaseDatePatch = await readFile(resolve(source, 'seed-purchase-date.js'), 'utf8');
if (!purchaseDatePatch.includes('type="date"') || !purchaseDatePatch.includes('purchaseDate')) throw new Error('Garden Labs exact Purchase Date patch is missing.');
...
console.log('Validated exact User Zero-approved source artwork, Garden Labs manifest, and exact Purchase Date patch.');
