...
export function CycleActions({ cycle, onChanged, onReplaced, selectedEvent, onOpenFact, onOpenObservation, maintenanceMode = false }: {
  ...
  return <form className="editor-card action-editor" onSubmit={(event) => void submit(event)}><div className="section-heading"><h2>{title[action]}</h2><button className="text-button" type="button" onClick={reset}>Cancelar</button></div>
    ...
    {(action === 'move') && <><p>Solo se muestran posiciones vacías de {cycle.garden.name}.</p><label>Nueva posición<select required value={targetPositionId} onChange={(event) => setTargetPositionId(event.target.value)}><option value="">Selecciona una posición</option>{garden?.positions.filter((position) => !position.current_cycle && position.id !== cycle.position.id).map((position) => <option key={position.id} value={position.id}>Posición {position.position_number}</option>)}</select></label><label>Fecha del traslado<input type="date" required value={date} onChange={(event) => setDate(event.target.value)} /></label></>}
