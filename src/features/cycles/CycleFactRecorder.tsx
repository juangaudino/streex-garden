...
export function CycleFactRecorder({ cycle, onSaved }: { cycle: GrowCycleDetail; onSaved: () => Promise<void> | void }) {
  ...
  if (!open) return <section className="cycle-fact-entry"><button className="secondary-button secondary-button--compact" type="button" onClick={() => setOpen(true)}><ClipboardPenLine size={17} aria-hidden="true" /> Registrar hecho</button><p className="quiet-copy">Germinación, conteos, evaluaciones e intervenciones confirmadas. Las recomendaciones no se registran aquí.</p></section>

  return <form className="editor-card action-editor" onSubmit={(event) => void submit(event)}>
    <div className="section-heading"><h2><Sprout size={19} aria-hidden="true" /> Registrar hecho</h2><button className="text-button" type="button" onClick={reset}>Cancelar</button></div>
    ...
    <button className="primary-button" type="submit" disabled={busy}>{busy ? 'Guardando…' : 'Guardar hecho'}</button>
  </form>
}
