...
function CycleScreen() {
  ...
  return <AppShell presentation="cycle" backTo={cycle ? `/garden/${cycle.garden.id}` : '/'} actions={<button className="secondary-button secondary-button--compact" type="button" onClick={() => void refreshCycle()} disabled={refreshing}><RefreshCw size={16} aria-hidden="true" /> {refreshing ? 'Actualizando…' : 'Actualizar'}</button>}>
    ...
    {cycle && <>
      ...
      <section className="history-section" aria-labelledby="history-title"><div className="section-heading"><h2 id="history-title">Historial</h2><span>{cycle.history.length}</span></div>{<Link className="history-compare-link" to={`/cycle/${cycle.id}/photos`}><Images size={17} aria-hidden="true" /> La historia de tu {cycle.crop_name}</Link>}
