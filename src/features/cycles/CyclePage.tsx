...
function CycleScreen() {
  ...
  return <AppShell presentation="cycle" backTo={cycle ? `/garden/${cycle.garden.id}` : '/'} actions={<button className="secondary-button secondary-button--compact" type="button" onClick={() => void refreshCycle()} disabled={refreshing}><RefreshCw size={16} aria-hidden="true" /> {refreshing ? 'Actualizando…' : 'Actualizar'}</button>}>
    ...
    {cycle && <>
      ...
      <section className="cycle-story-entry"><div><span>Historia de la planta</span><h2>La historia de tu {cycle.crop_name}</h2><p>{cycle.history.filter((event) => event.photo?.upload_status === 'uploaded').length === 0 ? 'Comienza aquí con la primera fotografía documental.' : cycle.history.filter((event) => event.photo?.upload_status === 'uploaded').length === 1 ? 'Ya hay un momento registrado. Recorre su evidencia.' : 'Recorre sus momentos y compara dos fotografías cuando quieras.'}</p></div><Link className="primary-button" to={`/cycle/${cycle.id}/photos`}><Images size={17} aria-hidden="true" /> Ver historia</Link></section>
