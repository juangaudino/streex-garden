...
function CycleScreen() {
  ...
  const [activityEditor, setActivityEditor] = useState<'fact' | 'observation' | null>(null)
  ...
  return <AppShell presentation="cycle" backTo={cycle ? `/garden/${cycle.garden.id}` : '/'} actions={<button className="secondary-button secondary-button--compact" type="button" onClick={() => void refreshCycle()} disabled={refreshing}><RefreshCw size={16} aria-hidden="true" /> {refreshing ? 'Actualizando…' : 'Actualizar'}</button>}>
    ...
    {cycle && <>
      ...
      <CycleActions key={eventToInvalidate?.id ?? 'actions'} cycle={cycle} selectedEvent={eventToInvalidate} onChanged={async () => { setEventToInvalidate(null); await load() }} onReplaced={(nextCycleId) => navigate(`/cycle/${nextCycleId}`)} onOpenFact={() => setActivityEditor('fact')} onOpenObservation={() => setActivityEditor('observation')} />
      {cycle.state === 'active' && activityEditor === 'fact' && <CycleFactRecorder cycle={cycle} initialOpen onSaved={async () => { setActivityEditor(null); await load() }} />}
