...
function PhotoLoader({ photo, eager, expandable, caption, actions, rendition }: { photo: PhotoEvidence; eager: boolean; expandable: boolean; caption: boolean; actions?: ReactNode; rendition: PhotoRendition }) {
  ...
  return <figure className="documentary-photo" ref={root}>
    ...
    {expandable && <dialog className="evidence-viewer" ref={dialog} onClose={afterClose} aria-label="Fotografía documental"><header><div><span className="evidence-viewer__eyebrow">REFERENCIA LOCAL</span><strong>La fotografía original</strong></div><button autoFocus type="button" className="icon-button" aria-label="Cerrar fotografía" onClick={close}><X /></button></header>{viewerUrl && !originalFailed ? <img src={viewerUrl} alt={`Original: ${photo.original_filename}`} onError={() => setOriginalFailed(true)} /> : <span className="documentary-photo__status">{originalFailed ? 'Original no disponible' : 'Abriendo original…'}</span>}<div className="evidence-viewer__meta"><strong>{captureLabel(photo)}</strong><span>Referencia local · {photo.original_filename} · {photo.content_type.replace('image/', '').toUpperCase()}</span></div>{actions && <div className="documentary-photo__actions">{actions}</div>}</dialog>}
  </figure>
}
