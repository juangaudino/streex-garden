    ...
    {cycle && <>
      ...
      {photos.length === 0 ? <StatePanel kind="empty" title="Aún no hay fotografías confirmadas">Growth Film aparecerá cuando este ciclo tenga evidencia fotográfica.</StatePanel> : <section ref={playerRef} className={`growth-film-player${immersive ? ' growth-film-player--immersive' : ''}`} aria-label="Reproductor de Growth Film">
        <div className="growth-film-frame" aria-live="polite">
          ...
          <div className="growth-film-frame__caption"><strong>{String(index + 1).padStart(2, '0')} · {captureLabel(current.photo)}</strong><span>{current.note ?? 'Evidencia fotográfica del ciclo.'}</span></div>
        </div>
        <div className="growth-film-controls"><button className="icon-button" type="button" aria-label={immersive ? 'Salir de pantalla completa' : 'Ver a pantalla completa'} onClick={() => void toggleImmersive()}>{immersive ? <Minimize2 size={17} /> : <Maximize2 size={17} />}</button><button className="secondary-button secondary-button--compact" type="button" onClick={() => setPlaying((value) => !value)}>{playing ? <Pause size={16} aria-hidden="true" /> : <Play size={16} aria-hidden="true" />}{playing ? 'Pausar' : 'Reproducir'}</button><input aria-label="Posición en Growth Film" type="range" min="0" max={photos.length - 1} value={index} onChange={(event) => { setPlaying(false); show(Number(event.target.value)) }} /><span>{index + 1}/{photos.length}</span></div>
        <ol className="growth-film-timeline">{photos.map((photo, photoIndex) => <li key={photo.id}><button type="button" className={photoIndex === index ? 'growth-film-timeline__point growth-film-timeline__point--active' : 'growth-film-timeline__point'} onClick={() => { setPlaying(false); show(photoIndex) }}><span>{String(photoIndex + 1).padStart(2, '0')}</span><small>{captureLabel(photo.photo)}</small></button></li>)}</ol>
      </section>}
