...
export function GrowthFilmStudio({ catalogue, backTo }: { catalogue: FilmCatalogue; backTo: string }) {
  ...
  const openEditor = (purpose: 'clip' | 'playback') => {
    generation.current += 1; setPending(false); setPlaying(false); setExpanded(false); setEditor(purpose)
  }
  return <div className={`botanical-surface film-page${reducedMotion ? ' film-page--reduced' : ''}`}>
    ...
    <main className="film-main"><div className="film-title"><div><span className="bs-eyebrow">{scope.kind === 'garden' ? 'EL DIARIO DEL JARDÍN' : current?.currentLocation ?? 'HISTORIA DE LA PLANTA'}</span><h1>El diario de <em>{scope.title}.</em></h1></div><p>Pequeños momentos.<br />Una historia que sigue creciendo.</p></div>
      {current ? <>
        <section className="cinema" aria-label="Reproductor de Growth Film"><div className="cinema-frame" aria-busy={pending || !urls[current.id]}>
          ...
          <span className="cinema-watermark">GARDEN X</span><span className="cinema-moment">{String(index + 1).padStart(2, '0')}<i> / {String(moments.length).padStart(2, '0')}</i></span><button type="button" className="bs-icon-button cinema-expand" aria-label="Ver fotografía en grande" disabled={!urls[current.id]} onClick={() => { setPlaying(false); setExpanded(true) }}><Maximize2 size={18} /></button>
    ...
    {expanded && current && urls[current.id] && <BotanicalSheet className="film-photo-sheet" title={current.title} context="FOTOGRAFÍA DEL MOMENTO" onClose={() => setExpanded(false)}><div className="film-photo-sheet__media"><img src={urls[current.id]} alt={`Fotografía ampliada de ${current.cropName}, ${current.dateLabel}`} /></div><div className="film-photo-sheet__meta"><span>{current.dateLabel}</span><span>{current.cycleLabel}</span></div><div className="bs-sheet-footer"><BotanicalButton secondary onClick={() => setExpanded(false)}>Volver al diario</BotanicalButton><BotanicalButton onClick={() => openEditor('playback')}>Ver película completa</BotanicalButton></div></BotanicalSheet>}
