...
import { Check, ChevronRight, ImagePlus, Leaf } from 'lucide-react'
...
function eventMarker(event: CycleHistoryEvent) {
  const completedAction = new Set([
    'action', 'harvest', 'intervention', 'system_maintenance', 'cycle_started',
    'cycle_ended', 'cycle_moved', 'seeds_added', 'incident_resolved',
  ]).has(event.event_type)
  const Icon = completedAction ? Check : Leaf
  return <span className={`timeline-marker${completedAction ? ' timeline-marker--action' : ''}`}><Icon size={16} /></span>
}
