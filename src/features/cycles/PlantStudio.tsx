...
export function PlantStudio({ cycle, onOpen, onCoverChanged, onMessage }: {
  ...
  return <div className="plant-page botanical-surface content-shell">
    <div className="page-utility"><Link className="breadcrumb" to={`/garden/${cycle.garden.id}`}>{cycle.garden.name}<ChevronRight size={13} />Posición {cycle.position.position_number}</Link><button className="bs-icon-button" type="button" aria-label="Opciones de la planta" onClick={() => onOpen({ kind: 'options' })}><MoreHorizontal size={21} /></button></div>
    <section className="plant-portrait" aria-label="Perfil del cultivo">
      <div className="portrait-image">{portrait ? <DocumentaryPhoto photo={portrait} eager expandable rendition="portrait" actions={<PhotoCoverActions gardenId={cycle.garden.id} cycleId={cycle.id} photoId={portrait.id} onMessage={onMessage} onChanged={onCoverChanged} />} /> : <div className="photo-placeholder"><Leaf size={34} /><span>Todavía sin fotografía documental</span></div>}<div className="portrait-position"><span>POSICIÓN</span><strong>{String(cycle.position.position_number).padStart(2, '0')}</strong></div><div className="portrait-reference">UNA MIRADA MÁS CERCA</div></div>
      <div className="plant-identity"><div className="bs-eyebrow"><span className="status-dot" />{cycle.state === 'active' ? 'CICLO ACTIVO' : 'CICLO CERRADO'}</div><h1>{name.common}</h1><p className="botanical-name">{name.subtitle}</p><p className="plant-lede">Cada hoja, parte de su historia.</p><div className="plant-facts"><div><strong>{planting.value}<span>{planting.unit}</span></strong><small>{planting.label}</small></div><div><strong>{photos.length}<span>fotografías</span></strong><small>disponibles en su diario</small></div></div>{cycle.state === 'active' && <BotanicalButton onClick={() => onOpen({ kind: 'choose' })}><Plus size={19} />Registrar un momento</BotanicalButton>}<button className="portrait-history-link" type="button" onClick={history}>Recorrer su historia<ArrowDown size={16} /></button></div>
    </section>
