...
export function GuestGardenStoryPage() {
  ...
  const history = useMemo(() => [...(story?.history ?? [])].sort((a, b) => Date.parse(b.occurred_at ?? '') - Date.parse(a.occurred_at ?? '')), [story])
  return <main className="guest-story-page"><header className="guest-story-page__header"><div className="guest-story-page__brand"><Leaf size={19} aria-hidden="true" /><span>Garden X</span></div><span className="guest-story-page__badge"><Eye size={15} aria-hidden="true" /> Solo lectura</span></header>
    {loading && <StatePanel kind="loading" title="Abriendo jardín compartido" />}{!loading && error && <section className="guest-story-page__state"><StatePanel kind="error" title="Jardín no disponible" onRetry={() => void load()}>{error}</StatePanel></section>}{!loading && story && <>
      <section className="guest-story-hero"><span className="eyebrow">Historia de un jardín</span><h1>{story.garden.name}</h1><p>{story.garden.system_model ?? 'Jardín hidropónico'} · {story.cycles.length} ciclos registrados</p><small>Vista compartida de solo lectura</small></section>
      <section className="guest-garden-cycles" aria-label="Ciclos del jardín"><h2>Sus plantas</h2><div className="guest-garden-cycle-grid">{story.cycles.map((cycle) => <article key={cycle.grow_cycle_id}><span>{String(cycle.position_number).padStart(2, '0')}</span><div><strong>{cycle.crop_name}</strong><small>Posición {cycle.position_number}{cycle.planted_on ? ` · ${new Intl.DateTimeFormat('es', { dateStyle: 'medium' }).format(new Date(`${cycle.planted_on}T12:00:00`))}` : ''}</small></div></article>)}</div></section>
      <section className="guest-story-timeline" aria-label="Historia del jardín">{history.length === 0 && <p className="empty-copy">Todavía no hay registros compartidos para este jardín.</p>}{history.map((event) => <article className="guest-story-event" key={event.id}><div className="guest-story-event__date"><strong>{label(event.event_type, event.event_data)}</strong><time>{event.occurred_at ? new Intl.DateTimeFormat('es', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(event.occurred_at)) : 'Fecha no registrada'}</time><small>{event.crop_name ?? 'Jardín'}{event.position_number ? ` · Posición ${event.position_number}` : ''}</small></div><div className="guest-story-event__body">{event.photo ? <figure className="guest-story-photo"><img src={event.photo.url} alt={`Fotografía documental: ${event.photo.original_filename}`} /><figcaption>{event.photo.original_filename}</figcaption></figure> : <ImageOff size={18} aria-label="Sin fotografía" />}{event.note && <p>{event.note}</p>}</div></article>)}</section>
