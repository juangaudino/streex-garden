...
import { getMaintenanceSession, progressMaintenancePosition, setMaintenanceSessionState } from '../../lib/garden-api'
...
export function MaintenancePage() {
  ...
  const reviewed = session?.positions.filter((position) => position.progress === 'reviewed').length ?? 0; const skipped = session?.positions.filter((position) => position.progress === 'skipped').length ?? 0
  return <AppShell presentation="maintenance" title="Mantenimiento" subtitle={session ? `${reviewed} revisadas · ${skipped} omitidas` : 'Cargando sesión'} backTo="/">
