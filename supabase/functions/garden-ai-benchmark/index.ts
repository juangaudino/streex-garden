  ...
  for (const model of models as string[]) {
    ...
    for (const fixture of selectedFixtures) {
      ...
      try {
        ...
        results.push({ fixture_id: fixture.id, model, idempotent: false, valid_structured_output: valid, latency_ms: Date.now() - startedAt, usage: response.usage ?? {}, estimated_cost_usd: response.usage ? Number((((response.usage.input_tokens ?? 0) / 1_000_000) * pricing[model].input + ((response.usage.output_tokens ?? 0) / 1_000_000) * pricing[model].output).toFixed(6)) : null, ...(valid ? {} : { raw_preview: rawText.slice(0, 600) }) })
      } catch (error) {
