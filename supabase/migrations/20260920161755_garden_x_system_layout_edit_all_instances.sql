-- Allow the same fixed-count geometry correction on official system instances.
-- Presets remain immutable; only the user's instance coordinates change.
begin;
...
begin
  ...
  select s.* into v_system from garden.system_instances s
  ...
  update garden.layout_sites set is_active = false, updated_at = now()
  ...
  for v_level in select value from jsonb_array_elements(p_levels) loop
    ...
    for v_row in 1..v_rows loop
      for v_column in 1..v_columns loop
        ...
        select p.id into v_position_id from garden.positions p
        where p.system_instance_id = v_system.id and p.position_number = v_position_number;
        if v_position_id is null then raise exception 'Position numbering is incomplete'; end if;
        update garden.layout_sites
        set level_number = v_level_number, row_number = v_row, column_number = v_column,
            grid_x = v_column, grid_y = v_row, updated_at = now()
  ...
  update garden.layout_sites set is_active = true, updated_at = now()
  where system_instance_id = v_system.id;
  if v_definition_id is null then
    update garden.system_instances
    set metadata = jsonb_set(metadata, '{layout_levels}', p_levels, true)
    where id = v_system.id and owner_id = v_owner;
  else
    update garden.custom_system_definitions set updated_at = now()
